# AGENTS.md — 多 Agent 协作执行规范

> 这是项目的"协作合同"——规定主编排 Agent、执行型 subagent / worker、外部模型（Codex / GLM / DeepSeek 等）、CLI 工具（aider）和你（产品负责人 + 终审）之间的责任划分。
>
...
### 步骤 3：你确认拆分 + 选模型
你看清单后说："1, 2 派低成本 worker；3 派 Sonnet/Codex worker；4 由主编排 Agent 亲自写。"
...
| **调试 Go race / 死锁** | Codex `gpt-5-xhigh` | 主编排 Agent | L3 复杂推理 |
...
| 复杂逻辑调试（race condition / 死锁） | 主编排 Agent | Codex `gpt-5-xhigh` / DeepSeek-R1 | L3，需要深推理 |
...
主编排 Agent **按"模块默认档位"自动派 Codex**，不每次问你。但下列信号触发时会**停下来跟你确认**：
...
- **常规任务**：主编排 Agent 按模块默认档位直接派 Codex，**事后告诉你用了哪档**
- **升降档信号触发**：派之前停下来跟你**确认档位**
- **你随时可以覆盖**：明确说"这个用 xhigh 跑"或"用 low 就够"，主编排 Agent 立即照做
