...
| 类型 | 例子 | 处理 |
|---|---|---|
| Invalid message | JSON 不合法、缺 task_id、schema 不兼容 | 后续进入 DLQ；当前阶段 worker 返回错误并保留未 ack |
...
DLQ 不在 3.10-3.15 中实现。后续实现时建议：

- 新增 `delivery.tasks.dlq.v1` topic。
- DLQ payload 包含原始 message、error_class、error_message、worker_id、failed_at。
- 只把不可恢复的 message 级错误写入 DLQ。
